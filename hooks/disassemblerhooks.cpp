...
KDDockWidgets::MainWindow* DisassemblerHooks::mainWindow() {
    return DisassemblerHooks::instance()->m_mainwindow;
}

DockWidget*
DisassemblerHooks::dockify(QWidget* w,
...
DockWidget*
DisassemblerHooks::tabify(QWidget* w,
                          KDDockWidgets::DockWidgetBase::Options options) {
    return DisassemblerHooks::tabify(DisassemblerHooks::dockify(w, options));
}
...
void DisassemblerHooks::focusOn(QWidget* w) {
    ...
    if(cw)
        static_cast<DockWidget*>(cw)->raise();
}
...
void DisassemblerHooks::open() {
    QString s =
        QFileDialog::getOpenFileName(m_mainwindow, tr("Disassemble file..."));
    if(!s.isEmpty())
        this->load(s);
}
...
void DisassemblerHooks::save() {
    ...
    if(ctx)
        this->saveAs(ctx, m_fileinfo.baseName() + CONTEXT_STATE_EXT);
}
...
void DisassemblerHooks::saveAs() {
    ...
    QString s = QFileDialog::getSaveFileName(m_mainwindow, tr("Save State..."),
                                             m_fileinfo.fileName(),
                                             "REDasm State (*.rds)");
    if(!s.isEmpty())
        this->saveAs(ctx, s);
...
void DisassemblerHooks::showFLC() {
    if(!m_flcdialog)
        m_flcdialog = new FLCDialog(m_mainwindow);
...
void DisassemblerHooks::showCallGraph(rd_address address) {
    ...
    this->showDialog(
        tr("Callgraph @ ") + QString::fromStdString(rd_tohex(address)), cgv);
    cgv->walk(address);
}
...
void DisassemblerHooks::showProblems() {
    ProblemsDialog dlgproblems(this->activeContext(), m_mainwindow);
    dlgproblems.exec();
}

SurfaceQt* DisassemblerHooks::activeSurface() {
    if(!DisassemblerHooks::activeContext())
        return nullptr;
    auto* activesurface =
        RDContext_GetActiveSurface(DisassemblerHooks::activeContext().get());
    return activesurface ? reinterpret_cast<SurfaceQt*>(
                               RDSurface_GetUserData(activesurface))
                         : nullptr;
}

RDContextPtr DisassemblerHooks::activeContext() {
    return DisassemblerHooks::instance()->m_disassemblerdocks
               ? DisassemblerHooks::instance()->m_disassemblerdocks->context()
               : nullptr;
}

void DisassemblerHooks::statusAddress(const SurfaceQt* surface) const {
    if(!surface || RDContext_IsBusy(surface->context().get()))
        return;
    ...
    QString segm = hassegment ? segment.name : tr("UNKNOWN"),
            offs = hassegment && offset.valid
                       ? RD_ToHexAuto(surface->context().get(), offset.value)
                       : tr("UNKNOWN"),
            addr = RD_ToHexAuto(surface->context().get(), address);

    QString s =
        QString::fromWCharArray(L"<b>Address: </b>%1\u00A0\u00A0").arg(addr);
    ...
    if(functionstart.valid && label) {
        ...
        if(func.size() > MAX_FUNCTION_NAME)
            func = func.left(MAX_FUNCTION_NAME) +
                   "..."; // Elide long function names

        if(address > functionstart.address)
            func += "+" + QString::fromUtf8(RD_ToHexBits(
                              address - functionstart.value, 8, false));
        else if(address < functionstart.address)
            func += QString::number(static_cast<std::make_signed<size_t>::type>(
                address - functionstart.value));

        s = QString::fromWCharArray(L"<b>Function: </b>%1\u00A0\u00A0")
                .arg(func.toHtmlEscaped()) +
            s;
    }
...
void DisassemblerHooks::hook() {
    ...
    connect(m_pbproblems, &QPushButton::clicked, this,
            [&]() { this->showProblems(); });

    connect(m_pbrenderer, &QPushButton::clicked, this, [&]() {
        const RDContextPtr& ctx = this->activeContext();
        RDContext_SetFlags(
            ctx.get(), ContextFlags_ShowRDIL,
            !RDContext_HasFlag(ctx.get(), ContextFlags_ShowRDIL));
...
void DisassemblerHooks::showLoaders(const QString& filepath, RDBuffer* buffer) {
    ...
    if(!RDContext_Bind(ctx.get(), &req, dlgloader.selectedLoaderEntry(),
                       dlgloader.selectedAssemblerEntry()))
        return;

    const RDLoader* loader = RDContext_GetLoader(ctx.get());
    const RDAssembler* assembler = RDContext_GetAssembler(ctx.get());
    rd_log(qUtf8Printable(
        tr("Selected loader '%1' with '%2' assembler")
            .arg(RDLoader_GetName(loader), RDAssembler_GetName(assembler))));

    rd_log(qUtf8Printable(tr("Minimum string length set to %1")
                              .arg(dlgloader.selectedMinString())));
...
void DisassemblerHooks::showWelcome() {
    DisassemblerHooks::tabify(new WelcomeWidget(),
                              KDDockWidgets::DockWidget::Option_NotClosable);
}

void DisassemblerHooks::loadRecents() {
    QMenu* mnurecents =
        m_mainwindow->findChild<QMenu*>(HOOK_ACTION_RECENT_FILES);
    if(!mnurecents)
        return;
    mnurecents->clear();
    ...
    connect(action, &QAction::triggered, this, [=]() { this->clearRecents(); });
}
...
void DisassemblerHooks::clearRecents() {
    ...
    if(!m_disassemblerdocks)
        this->close(true); // Recreate Welcome Tab
}
...
void DisassemblerHooks::load(const QString& filepath) {
    ...
    if(this->openDatabase(filepath))
        return;

    RDBuffer* buffer = RDBuffer_CreateFromFile(qUtf8Printable(filepath));
    if(buffer && RDBuffer_Size(buffer))
        this->showLoaders(filepath, buffer);
    else if(buffer)
        RDObject_Free(buffer);
}

bool DisassemblerHooks::isLoaded() const {
    return m_disassemblerdocks != nullptr;
}

QAction* DisassemblerHooks::addWindowAction(DockWidget* dw) {
    if(!dw || !m_mnuwindow || m_windowactions.size() == MAX_WINDOW_ACTIONS)
        return nullptr;
...
OutputWidget* DisassemblerHooks::outputWidget() const {
    return m_dockoutput ? static_cast<OutputWidget*>(m_dockoutput->widget())
                        : nullptr;
}
...
void DisassemblerHooks::checkListingMode() {
    if(RDContext_HasFlag(this->activeContext().get(), ContextFlags_ShowRDIL))
        m_pbrenderer->setText(tr("RDIL"));
    else
        m_pbrenderer->setText(tr("Listing"));
}

void DisassemblerHooks::showOutput() {
    m_dockoutput = DisassemblerHooks::dockify(
        new OutputWidget(m_mainwindow),
        KDDockWidgets::DockWidget::Option_NotClosable);
...
void DisassemblerHooks::close(bool showwelcome) {
    ...
    if(m_devdialog)
        m_devdialog->deleteLater();
    m_devdialog = nullptr;

    auto docks = m_mainwindow->findChildren<KDDockWidgets::DockWidget*>(
        QString(), Qt::FindChildrenRecursively);

    std::for_each(docks.begin(), docks.end(),
                  [](KDDockWidgets::DockWidget* dw) {
                      if(dynamic_cast<OutputWidget*>(dw->widget()))
                          return;
                      dw->deleteLater();
                  });

    if(showwelcome)
        this->showWelcome(); // Replaces central widget, if any
    if(m_disassemblerdocks)
        m_disassemblerdocks->deleteLater();
    m_disassemblerdocks = nullptr;
}

const DisassemblerDocks* DisassemblerHooks::docks() {
    return DisassemblerHooks::instance()->m_disassemblerdocks;
}
...
void DisassemblerHooks::enableMenu(QMenu* menu, bool enable) {
    ...
    std::for_each(actions.begin(), actions.end(),
                  [enable](QAction* a) { a->setEnabled(enable); });

    if(dynamic_cast<QMenuBar*>(menu->parentWidget()))
        menu->menuAction()->setVisible(enable);
    menu->setEnabled(enable);
}

void DisassemblerHooks::saveAs(const RDContextPtr& ctx,
                               const QString& filepath) {
    if(RDContext_Save(ctx.get(), qUtf8Printable(filepath)))
        rd_log(tr("Saving Database '%1'").arg(filepath).toStdString());
}

bool DisassemblerHooks::openDatabase(const QString& filepath) { return false; }
...
void DisassemblerHooks::openHomePage() const {
    QDesktopServices::openUrl(QUrl("https://redasm.io"));
}
void DisassemblerHooks::openTwitter() const {
    QDesktopServices::openUrl(QUrl("https://twitter.com/re_dasm"));
}
void DisassemblerHooks::openTelegram() const {
    QDesktopServices::openUrl(QUrl("https://t.me/REDasmDisassembler"));
}
void DisassemblerHooks::openReddit() const {
    QDesktopServices::openUrl(QUrl("https://www.reddit.com/r/REDasm"));
}
void DisassemblerHooks::openGitHub() const {
    QDesktopServices::openUrl(
        QUrl("https://github.com/REDasmOrg/REDasm/issues"));
}

void DisassemblerHooks::showMessage(const QString& title, const QString& msg,
                                    size_t icon) {
...
void DisassemblerHooks::updateViewWidgets(bool busy) {
    ...
    if(busy) {
        m_mainwindow->setWindowTitle(
            QString("%1 (%2)").arg(m_fileinfo.fileName(), tr("Working")));
    ...
    if(this->activeContext()) {
        m_pbproblems->setVisible(
            !busy && RDContext_HasProblems(this->activeContext().get()));
        m_pbproblems->setText(QString::number(RDContext_GetProblemsCount(
                                  this->activeContext().get())) +
                              tr(" problem(s)"));
    }
    else
        m_pbproblems->setVisible(false);
...
void DisassemblerHooks::enableCommands(QWidget* w) {
    QAction* actdevtools =
        m_mainwindow->findChild<QAction*>(HOOK_ACTION_DEVTOOLS);
