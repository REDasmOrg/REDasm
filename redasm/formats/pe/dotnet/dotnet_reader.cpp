...
void DotNetReader::iterateTypes(MethodCallback cbmethods) const
{
    ...
    {
        u32 c = this->getListCount(it, CorMetadataTables::MethodDef, [](const CorTable& cortable) -> u32 {
...
void DotNetReader::iterateMethods(const CorTable& cortypedef, u32 methodcount, MethodCallback cbmethods) const
{
    std::string tname;
...
u32 DotNetReader::getListCount(CorTableRows::const_iterator it, u32 cortable, IndexCallback cbindex) const
{
    const CorTableRows& cortablerows = this->getTableRows(cortable);
    u32 index = cbindex(*it), nextindex = cortablerows.size();
    it++;

    if(it != cortablerows.end())
        nextindex = cbindex(*it);
