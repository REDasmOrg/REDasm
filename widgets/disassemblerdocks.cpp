...
void DisassemblerDocks::showSegments() const {
    TableWidget* tw =
        this->createTable(new SegmentsModel(m_context), tr("Segments"));
    connect(tw, &TableWidget::resizeColumns, tw,
            &TableWidget::resizeAllColumns);
    DisassemblerHooks::tabify(tw);
}

void DisassemblerDocks::showFunctions() const {
    TableWidget* tw =
        this->createTable(new FunctionsModel(m_context), tr("Functions"));
    connect(tw, &TableWidget::resizeColumns, this,
            [tw]() { tw->resizeColumn(0); });

    auto* dock = DisassemblerHooks::dockify(tw);
    DisassemblerHooks::mainWindow()->addDockWidget(
        dock, KDDockWidgets::Location_OnLeft, nullptr, tw->sizeHint());
}

void DisassemblerDocks::showExports() const {
    TableWidget* tw = this->createTable(
        new LabelsModel(m_context, AddressFlags_Exported), tr("Exports"));
    connect(tw, &TableWidget::resizeColumns, tw,
            &TableWidget::resizeAllColumns);
    DisassemblerHooks::tabify(tw);
}
...
void DisassemblerDocks::showImports() const {
    TableWidget* tw = this->createTable(
        new LabelsModel(m_context, AddressFlags_Imported), tr("Imports"));
    connect(tw, &TableWidget::resizeColumns, tw,
            &TableWidget::resizeAllColumns);
    DisassemblerHooks::tabify(tw);
}
...
void DisassemblerDocks::showStrings() const {
    TableWidget* tw =
        this->createTable(new StringsModel(m_context), tr("Strings"));
    connect(tw, &TableWidget::resizeColumns, tw,
            &TableWidget::resizeAllColumns);
    DisassemblerHooks::tabify(tw);
}
...
void DisassemblerDocks::showMap(KDDockWidgets::DockWidget* relative) const {
    ...
    DisassemblerHooks::mainWindow()->addDockWidget(
        mapdock, KDDockWidgets::Location_OnRight, relative, opt);
}
...
TableWidget* DisassemblerDocks::createTable(ContextModel* model,
    ...
    connect(tw, &TableWidget::doubleClicked, this,
            &DisassemblerDocks::onItemDoubleClicked);
    return tw;
}
    ...
    switch(e->id) {
        case Event_BusyChanged:
            QMetaObject::invokeMethod(
                DisassemblerHooks::instance(), "updateViewWidgets",
                Qt::QueuedConnection,
                Q_ARG(bool, RDContext_IsBusy(thethis->m_context.get())));
            break;

        case Event_Error: {
            const auto* ee = reinterpret_cast<const RDErrorEventArgs*>(e);
            QMetaObject::invokeMethod(DisassemblerHooks::instance(),
...
DockWidget* DisassemblerDocks::showListing() const {
    return DisassemblerHooks::tabify(new ListingSplitView(m_context));
}

void DisassemblerDocks::setContext(const RDContextPtr& ctx) {
    m_context = ctx;
    auto* analysiswidget = new AnalysisWidget(ctx);
    m_analysisdock = DisassemblerHooks::dockify(
        analysiswidget, KDDockWidgets::DockWidget::Option_NotClosable);
    ...
    RDObject_Subscribe(ctx.get(), this, &DisassemblerDocks::listenEvents,
                       nullptr);
